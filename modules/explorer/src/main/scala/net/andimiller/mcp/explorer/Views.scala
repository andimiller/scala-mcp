...
object Views:
          ...
          div(`class` := "field is-grouped")(
            div(`class` := "control is-expanded")(
              input(
                ...
                onInput(s => Msg.SetUrl(s))
              )
            ),
            div(`class` := "control")(
              model.connection match
                case ConnectionStatus.Disconnected =>
                  button(`class` := Styles.connectButton, onClick(Msg.Connect))("Connect")
  ...
  private def toolView(model: Model, tool: ToolDefinition): Html[Msg] = {
    ...
    div(
      ...
      div(`class` := Styles.section)(
        ...
        textarea(
          `class` := (if hasErr then Styles.textareaError else Styles.textarea),
          value   := model.toolInputs.getOrElse(n, "{}"),
          onInput(s => Msg.UpdateToolInput(n, s))
        )(),
      ...
      model.toolResults.get(n) match
        case Some(r) =>
          div(`class` := Styles.section)(
            div(`class` := Styles.sectionTitle)("Result"),
            div(`class` := Styles.resultBox)(r.content.map(contentText).mkString("\n"))
          )
        case None => span()
    )
  }
  ...
  private def resourceView(model: Model, r: ResourceDefinition): Html[Msg] = {
    val u = r.uri
    div(
      div(`class` := Styles.detailTitle)(r.name),
      div(`class` := Styles.detailDescription)(r.description.getOrElse("")),
      div(`class` := "mb-2")(code(u)),
      if model.resourceErrors.contains(u) then div(`class` := Styles.errorText)(model.resourceErrors(u))
        ...
        div(
          ...
          div(`class` := Styles.section)(
            div(`class` := Styles.sectionTitle)("Content"),
            pre(`class` := Styles.resourceContent)(
              model.resourceContents(u).contents.map(_.text.getOrElse("[No text]")).mkString("\n")
            )
          )
        )
    ...
    div(
        ...
        div(`class` := "field has-addons")(
          div(`class` := "control is-expanded")(
            input(
              ...
              placeholder := tpl,
              value       := uri,
              onInput(s => Msg.UpdateTemplateUri(tpl, s))
            )
          ),
          div(`class` := "control")(
      ...
      if model.resourceErrors.contains(readUri) then div(`class` := Styles.errorText)(model.resourceErrors(readUri))
        ...
        div(`class` := Styles.section)(
          div(`class` := Styles.sectionTitle)("Content"),
          pre(`class` := Styles.resourceContent)(
            model.resourceContents(readUri).contents.map(_.text.getOrElse("[No text]")).mkString("\n")
          )
        )
      else span()
    )
  ...
  private def promptView(model: Model, p: PromptDefinition): Html[Msg] = {
    ...
    div(
      div(`class` := Styles.detailTitle)(n),
      div(`class` := Styles.detailDescription)(p.description.getOrElse("")),
      if p.arguments.nonEmpty then
        div(`class` := Styles.section)(
          List(div(`class` := Styles.sectionTitle)("Arguments")) ++
            p.arguments.map(a =>
              div(`class` := "field mb-3")(
                label(`class` := "label")(s"${a.name}${if a.required then " *" else ""}"),
                div(`class` := "control")(
                  input(
                    `class`     := "input",
                    `type`      := "text",
                    placeholder := a.description.getOrElse(""),
                    value       := model.promptArgs.getOrElse(n, Map.empty).getOrElse(a.name, ""),
                    onInput(s => Msg.UpdatePromptArg(n, a.name, s))
                  )
                )
              )
            )
      ...
      else if model.promptResults.contains(n) then
        div(`class` := Styles.section)(
          div(`class` := Styles.sectionTitle)("Messages"),
          div(`class` := Styles.promptMessages)(
            model
              .promptResults(n)
              .messages
              .map(m =>
                div(`class` := Styles.promptMessage)(
                  div(`class` := Styles.promptRole)(m.role.toString),
                  pre(`class` := "is-family-monospace")(contentText(m.content))
                )
              )
          )
        )
