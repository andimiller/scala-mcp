 ...
 *
 * All state for a session is stored as fields in a single hash at
 * `mcp:session:{sessionId}`. Each `ref` call creates (or reuses) a field
 * within that hash. If the field already exists (session reconstruction),
 * the stored value is used; otherwise the provided `initial` value is written.
 */
