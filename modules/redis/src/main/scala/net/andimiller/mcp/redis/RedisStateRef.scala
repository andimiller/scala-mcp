 ...
 *
 * All state for a session lives in a single hash at `hashKey`, with each
 * piece of state stored as a separate field.
 *
 * '''Important:''' `update` and `modify` are NOT atomic across servers — they perform
 * read-then-write. With session affinity (the expected deployment model) this is safe.
 */
