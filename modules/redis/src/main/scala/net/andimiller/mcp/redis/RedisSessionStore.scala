...
import net.andimiller.mcp.http4s.{AuthenticatedSessionStore, McpSession, SessionStore, SessionStoreFactory}

import scala.concurrent.duration.FiniteDuration

/**
 * A [[SessionStore]] that uses a Redis hash for session registry (with TTL)
 * and a local in-memory cache for live `McpSession` objects (which contain
 * non-serializable handles like request handlers and notification sinks).
 *
 * Each session's hash at `mcp:session:{id}` contains an `_active` field
 * as well as any state fields written by [[RedisSessionRefs]].
 *
 * When a session is not in the local cache but the hash exists in Redis,
 * the `reconstruct` callback rebuilds the session (creating new Redis-backed
 * sinks and state refs that automatically pick up the persisted state fields).
 */
...
/**
 * A [[AuthenticatedSessionStore]] that extends [[RedisSessionStore]] with
 * Redis-backed user identity storage (stored as a field in the session hash).
 */
