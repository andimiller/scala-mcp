package net.andimiller.mcp.examples
...
object WeatherServer extends IOApp.Simple:
  ...
  def run: IO[Unit] =
    for
      ...
      // Test tool listing
      tools <- server.listTools(net.andimiller.mcp.core.protocol.ListToolsRequest())
      ...
      // Test calling a tool
      _ <- IO.println("\nTesting tool call...")
      request = net.andimiller.mcp.core.protocol.CallToolRequest(
        name = "get_weather",
