  ...
  def roll(expr: DiceParser.DiceExpr): F[(Int, String)] =
    expr match
      case DiceParser.Roll(dice) =>
        List.fill(dice.count)(random.nextIntBounded(dice.sides).map(_ + 1)).sequence.map { rolls =>
          val total     = rolls.sum
      ...
      case Right(expr) =>
        ...
        roll(expr).map { (result, breakdown) =>
          DiceRoller.RollResult(
            expression = expression, min = min, max = max, result = result, breakdown = breakdown
          )
        }
