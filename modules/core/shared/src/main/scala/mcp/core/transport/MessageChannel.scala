package mcp.core.transport

import cats.effect.kernel.Async
import fs2.Stream
import mcp.core.protocol.jsonrpc.Message

/**
 * Transport-agnostic abstraction for bidirectional message communication.
 *
 * This allows different transports (stdio, HTTP+SSE, WebSocket, etc.) to be used
 * interchangeably with the same server implementation.
 */
...
object MessageChannel:
  /**
   * Create a message channel from explicit send/receive functions.
   * Useful for testing and custom transports.
   */
  def apply[F[_]: Async](
