 ...
 *
 * Form mode only — the server sends a JSON Schema describing the data it wants from the user;
 * the client renders a form, the user submits, and the response is decoded back to `A`.
 *
 * The schema is derived from `JsonSchema[A]` (existing project typeclass) and the response
 * is decoded via `io.circe.Codec[A]`.
 */
trait ElicitationClient[F[_]]:
   ...
   *
   * @param message  human-readable prompt shown alongside the form
   * @param timeout  optional bound on how long to wait for the user to respond
   */
...
object ElicitationClient:
  ...
  private def decodeEnvelope[A](envelope: ElicitationCreateResponse)(using codec: Codec[A]): Either[ElicitationError, ElicitResult[A]] =
