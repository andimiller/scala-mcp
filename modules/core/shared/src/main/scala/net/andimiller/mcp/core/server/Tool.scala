...
object Tool:
  ...
  /** Build a [[Resolved]] from a [[ToolDefinition]] and a raw handler that produces a `ToolResult[Nothing]`. Used by
    * adapters (e.g. openapi-proxy) that supply their own `outputSchema` on the definition and only emit
    * `Raw`/`Text`/`Error` branches.
    */
...
object ToolBuilder:
  ...
  extension [In, Out](b: WithIn[Unit, In, Out])
    ...
    /** Plain (`Ctx = Unit`) [[run]]: auto-calls `.provide(())` and returns a resolved handler ready to feed into
      * `withTool`.
      */
    def run[F[_]: Async](handler: In => F[Out]): Tool.Resolved[F] =
      b.runResult[F]((in: In) => handler(in).map(ToolResult.Success(_)))

    /** Plain (`Ctx = Unit`) [[runResult]]: auto-calls `.provide(())` and returns a resolved handler ready to feed into
      * `withTool`.
      */
