...
/** Result of a tool execution, parameterised on the success-value type so the builder DSL can auto-derive the output
  * schema.
  *
  * Branches:
  *   - `Success(value)` — structured success; encoded as both human-readable text (the JSON stringified) and
  *     `structuredContent` (the JSON itself).
  *   - `Text(text)` — free-form text response.
  *   - `Error(message)` — failure with a human-readable message; sets `isError = true`.
  *   - `Raw(content, structuredContent, isError)` — escape hatch for advanced cases (custom content lists, structured
  *     payloads that aren't object-shaped, errors with structured payloads). Used by openapi-proxy whose success type
  *     is arbitrary `Json`.
  */
enum ToolResult[+R]:
...
/** Resolves the output-schema question for a given `R`:
  ...
  *
  * Used by the tool builders so a single `.runResult[R]` API works for both schema-bearing and schema-less handlers.
  */
...
object OutputSchema:
  ...
  /** Higher-priority instance for the bottom type — pinned in the companion object so it's preferred when
    * `R = Nothing`.
    */
