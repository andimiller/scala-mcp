...
/** Bundle of every server→client communication primitive.
  *
  * Both [[NotificationSink]] (one-way notifications) and [[ServerRequester]] (request/response with correlation)
  * publish to a single shared `fs2.concurrent.Topic[F, Message]`. The [[subscribe]] stream is the canonical outbound
  * feed consumed by the SSE channel (HTTP) or the stdout writer (stdio).
  *
  * Server factories receive a `ClientChannel[F]` rather than a `NotificationSink[F]`, so adding future server-initiated
  * features (sampling, roots/list) does not break factory signatures.
  */
...
object ClientChannel:
  ...
  /** Build a ClientChannel that wraps an existing [[NotificationSink]] (e.g. a user-provided Redis-backed sink). The
    * requester gets its own internal Topic; the [[subscribe]] stream merges both so a single outbound feed (SSE /
    * stdout) carries both notifications and server-initiated requests.
    */
