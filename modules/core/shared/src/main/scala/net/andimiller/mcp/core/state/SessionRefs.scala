 ...
 *
 * The `name` parameter uniquely identifies a piece of state within a session.
 * For external backends (e.g. Redis), this becomes part of the storage key.
 *
 * The `Encoder`/`Decoder` constraints are required by external backends for serialization
 * but ignored by the in-memory implementation.
 */
