...
private[http4s] object HttpMcpRouting:
  ...
  def serveAuthenticated[U: Eq](
    authenticate: Request[IO] => IO[Option[U]],
    serverFactory: (U, NotificationSink[IO]) => IO[McpServer[IO]],
    ...
    StreamableHttpTransport.authenticatedRoutes[IO, U](authenticate, serverFactory, IO(onUnauthorized)).flatMap { mcpRoutes =>
      val redirectRoute = HttpRoutes.of[IO] {
        case GET -> Root => IO.pure(Response[IO](Status.Found).withHeaders(Location(uri"/explorer/index.html")))
      }
      val routes =
