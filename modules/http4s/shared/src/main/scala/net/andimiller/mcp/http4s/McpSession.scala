...
import net.andimiller.mcp.core.server.{ClientChannel, RequestHandler}

/**
 * State for a single MCP client session over HTTP.
 *
 * Each session has its own request handler, client channel (notifications + server-initiated
 * requests), and subscription set.
 */
