...
/** Transport implementation using stdin/stdout for communication.
  *
  * Messages are sent as newline-delimited JSON over stdio. This is the standard transport for MCP servers that are
...
object StdioTransport:
  ...
  /** Run a server built from a [[SessionContext]] factory — the unified shape that grants access to the per-session
    * [[ClientChannel]] (notifications + server-initiated requests such as `elicitation/create`) and an in-memory
    * [[SessionRefs]].
    *
